...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Trio, please download the latest version! [Trio Editor](https://voideditor.com/download-beta)!'
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Trio Error: There was an error checking for updates. If this persists, please get in touch or reinstall Trio [here](https://voideditor.com/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'Trio: Check for Updates'),
		});
	}
